...
function ridge_regression(y::Vector{T}, t::Vector{T}; λ=0::Real) where {T<:Real}
    t = t .- t[1]
    T_bias_ext = hcat( t, ones(length(t)) )'
    return inv(T_bias_ext * T_bias_ext' + λ .* I(2)) * T_bias_ext * y
end

function ridge_regression(Y::Matrix{T}, t::Vector{T}; λ=0::Real) where {T<:Real}
    t = t .- t[1]
    T_bias_ext = hcat( t, ones(length(t)) )'
    return inv(T_bias_ext * T_bias_ext' + λ .* I(2)) * T_bias_ext * transpose(Y)
end

function ridge_regression_slope(Y::Matrix{T}, t::Vector{T}; λ=0::Real) where {T<:Real}
    return ridge_regression(Y, t; λ)[1, :]
end

# GPU version of ridge regression. Y must be nt x ns.
function ridge_regression(Y::CuArray{T, 2}, t::Vector{T}; λ=0::Real) where {T<:Real}
    t = t .- t[1]
    T_bias_ext = hcat( t, ones(length(t)) )'
    return CuArray( inv(T_bias_ext * T_bias_ext' + λ .* I(2)) ) * CuArray( T_bias_ext ) * permutedims(Y)
end

function ridge_regression_slope(Y::CuArray{T, 2}, t::Vector{T}; λ=0::Real) where {T<:Real}
    return ridge_regression(Y, t; λ)[1, :]
end
